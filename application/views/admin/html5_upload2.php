<input id="setting-img-upload" type="file" accept="image/*" capture="camera" style="display: none">
<canvas id="cvs" width="0" height="0" class="hidden"></canvas>
<script>
var pos;
function myclick(d) {
    pos = d;
    document.getElementById('setting-img-upload').click();
    console.log('myclick' + pos);
}
//绑定input change事件
$("#setting-img-upload").unbind("change").on("change",function(){
    console.log(pos);
...
function putb64(pic){
	...
	xhr.onreadystatechange=function(){
		if (xhr.readyState==4){
            ...
            filename = obj.key;
            var tmp = '\
                <img src="<?= img_url() ?>' + filename + '" style="width: 100%">\
                <input type="hidden" name="img" value="' + filename + '">';
            document.getElementById('setting-img-preview-' + pos).innerHTML = tmp;
		}
	}
